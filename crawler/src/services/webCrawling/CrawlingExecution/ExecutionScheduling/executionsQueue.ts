...
export default class ExecutionsPriorityQueue {
	...
	private PopUnTimedQItem(): ExecutionsRecord | undefined {
		...
		if (this.sizeUnTimedQ == 1) this.tailUnTimed = undefined;
	...
	private PopTimedQItem(): ExecutionsRecord | undefined {
		...
		if (this.sizeTimedQ == 1) this.tailTimed = undefined;
	...
	private PushToTimedQueue(newNode: QueueNode) {
		...
		while (
			...
			this.IsNewRecOlderThan(
				newNode.Data as ExecutionsRecord,
				currentNode
			)
		) {
		...
		if (currentNode == this.tailTimed) this.tailTimed = newNode;
	...
	private PushToUnTimedQueue(newNode: QueueNode) {
		...
		if (this.tailUnTimed) this.tailUnTimed.Next = newNode;
	...
	private IsNewRecOlderThan(
		executionRecord: ExecutionsRecord,
		nodeToCompareWith: QueueNode
	): boolean {
		if (
			!executionRecord.TimeToExecute ||
			!nodeToCompareWith.Data?.TimeToExecute
		)
			return false;
